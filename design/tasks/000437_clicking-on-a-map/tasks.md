...
- [x] Pass `lat`/`lng` through the birds API route (`app/api/birds/route.ts`) — eBird returns them but `BirdResponse` currently drops them.
- [x] Add `latitude`/`longitude` to the `Bird` type (`lib/types/Bird.ts`) and the bird client (`lib/api/birdClient.ts`).
- [x] Update `BirdCard.tsx` to display coordinates in the location snippet: `📍 Smug Oak Lane, Colney Street, England, GB (51.709, -0.336)` and make it clickable to open the map modal.