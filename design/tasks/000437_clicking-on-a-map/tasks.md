...
- [~] Modify `components/LocationList.tsx` — pass `latitude` and `longitude` through to `LocationCard` in the props.
- [~] Import Leaflet CSS (either in the map component or via a link tag in the modal) so tiles and markers render correctly.
- [ ] Verify mobile responsiveness — map modal should be near full-screen on small viewports (~90vw × 80vh on desktop).
- [ ] Manual test: search a postcode, click "View on map" on a location card, confirm marker is pinned at the correct spot, pan/zoom works, and modal dismisses cleanly.